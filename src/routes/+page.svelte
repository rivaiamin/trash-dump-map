<script lang="ts">
	...
	function detectFacilityType(amenity: string, name?: string): Facility['type'] {
		...
		if (name) {
			...
			if (
				lowerName.includes('tempat sampah') ||
				lowerName.includes('pembuangan') ||
				lowerName.includes('sampah')
			)
				return 'trash';
		}
...
<div class="min-h-screen bg-gradient-to-br from-blue-50 to-green-50">
	<div class="container mx-auto p-4 space-y-6">
		<div class="text-center py-6">
			<div
				class="inline-flex items-center justify-center w-16 h-16 bg-gradient-to-br from-blue-500 to-green-500 rounded-full mb-4"
			>
		...
		<div class="flex flex-col sm:flex-row items-center gap-3">
			...
			{#if permissionDenied}
				<div
					class="flex items-center gap-2 text-sm text-amber-600 bg-amber-50 px-3 py-2 rounded-lg border border-amber-200"
				>
		...
		<div class="relative">
			...
			{#if suggestions.length > 0}
				<ul
					class="absolute z-20 mt-2 w-full bg-white border border-gray-200 rounded-lg shadow-lg divide-y divide-gray-100 max-h-60 overflow-y-auto"
					role="listbox"
				>
					{#each suggestions as s, i}
						<li
							class="px-4 py-3 cursor-pointer hover:bg-blue-50 transition-colors duration-150 {i ===
							activeIndex
								? 'bg-blue-100'
								: ''}"
							role="option"
		...
		<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
			...
			<div class="lg:col-span-2 relative">
				...
				{#if isLoadingFacilities}
					<div
						class="absolute inset-0 bg-white/80 backdrop-blur-sm grid place-items-center rounded-xl"
					>
				...
				{#if showingRoute}
					<div class="absolute top-4 right-4 flex flex-col gap-2">
						<button
							class="inline-flex items-center gap-2 h-10 px-4 rounded-lg bg-white border border-gray-200 shadow-lg hover:shadow-xl focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2 text-sm font-medium text-gray-700 transition-all duration-200"
							on:click={clearRoute}
						>
							<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
								<path
			...
			<div class="lg:col-span-1">
				<div
					class="bg-white rounded-xl shadow-lg border border-gray-200 p-6 h-fit lg:sticky lg:top-6 lg:h-[70vh] lg:flex lg:flex-col"
				>
					<div class="flex items-center gap-3 mb-4 flex-shrink-0">
						<div
							class="w-8 h-8 bg-gradient-to-br from-green-500 to-blue-500 rounded-lg flex items-center justify-center"
						>
						...
						{#if facilities.length > 0}
							<span class="bg-blue-100 text-blue-800 text-xs font-medium px-2.5 py-0.5 rounded-full"
								>{facilities.length} found</span
							>
						{/if}
					</div>

					<div class="flex-1 lg:overflow-y-auto lg:min-h-0">
						{#if facilities.length === 0}
							<div class="text-center py-8">
								...
								<p class="text-sm text-gray-400">
									Try "Use my location" or search for an address to find nearby facilities
								</p>
							</div>
						{:else}
							<ul
								class="space-y-3"
								on:keydown={onFacilitiesKeydown}
								tabindex="0"
								role="listbox"
								aria-activedescendant={selectedFacility?.id}
							>
							{#each facilities as f, i}
								<li
									id={f.id}
									class="group p-4 rounded-lg border border-gray-200 cursor-pointer transition-all duration-200 hover:shadow-md hover:border-blue-300 focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2 {selectedFacility?.id ===
									f.id
			...
			>
				<div class="bg-gradient-to-r from-blue-600 to-blue-700 text-white p-4">
					<div class="flex items-center justify-between">
						<div class="flex items-center gap-3">
							...
							<h3 class="font-semibold">Route Directions</h3>
						</div>
						<button
							class="h-8 w-8 rounded-lg hover:bg-white/20 focus-visible:ring-2 focus-visible:ring-white/50 focus-visible:ring-offset-2 transition-colors duration-200 flex items-center justify-center"
							on:click={clearRoute}
					...
					{#if routeSummary}
						<div class="mt-2 flex items-center gap-4 text-sm">
							...
							<div class="flex items-center gap-1">
								...
								<span>{routeSummary.distance} km</span>
							</div>
						</div>
					{/if}
				</div>
				<div class="max-h-64 overflow-y-auto">
					<div class="p-4 space-y-3">
						{#each routeInstructions as instruction, i}
		...
		{#if selectedFacility}
			<div
				class="fixed bottom-4 left-1/2 -translate-x-1/2 w-[calc(100%-2rem)] max-w-screen-sm lg:bottom-6 lg:right-6 lg:left-auto lg:translate-x-0 lg:w-1/3 lg:max-w-none bg-white border border-gray-200 shadow-2xl rounded-xl p-4 transform transition-all duration-300 ease-out"
			>
						...
						<div class="flex-1 min-w-0">
							...
							<div class="flex items-center gap-1 mt-1">
								...
								<span class="text-sm text-gray-600"
									>{formatDistance(selectedFacility.distanceKm)}</span
								>
							</div>
						</div>
					</div>
